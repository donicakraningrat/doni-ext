...
export default function Query({ apiEndpoint, config }: { apiEndpoint: string, config: TConfig }) {
  const [queryList, setQueryList] = useLocalStorage<TQuery>("TQuery", {
    db: "users",
    table: "users",
    query: "select * from users.users where email = ? limit 10;",
    where: ""
  });
  ...
  return (
    <>
      ...
      <TextArea id="result" value={JSON.stringify(queryList)} onChange={e => { }} />
    </>
  );
}